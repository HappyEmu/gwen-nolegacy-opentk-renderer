...
namespace Gwen.UnitTest
{
    public class TreeControl : GUnit
    {
        ...
        private void NodeCollapsed(Base control, EventArgs args)
        {
            TreeNode node = control as TreeNode;
            UnitPrint(String.Format("Node collapsed: {0}", node.Text));
        }

        private void NodeExpanded(Base control, EventArgs args)
        {
            TreeNode node = control as TreeNode;
            UnitPrint(String.Format("Node expanded: {0}", node.Text));
        }

        private void NodeSelected(Base control, EventArgs args)
        {
            TreeNode node = control as TreeNode;
            UnitPrint(String.Format("Node selected: {0}", node.Text));
        }

        private void NodeClicked(Base control, ClickedEventArgs args)
        {
            TreeNode node = control as TreeNode;
            UnitPrint(String.Format("Node clicked: {0} @({1}, {2})", node.Text, args.X, args.Y));
        }

        private void NodeDoubleClicked(Base control, ClickedEventArgs args)
        {
            TreeNode node = control as TreeNode;
